...
def lstm_cell_forward(xt, a_prev, c_prev, parameters):
    ...
    ft = sigmoid(np.add(np.matmul(Wf,concat),bf))
    it = sigmoid(np.add(np.matmul(Wi,concat),bi))
    cct = np.tanh(np.add(np.matmul(Wc,concat),bc))
    #remmeber c_next is element wise multiply *, not using np.dot or np.matmul
    c_next = np.add(np.multiply(ft,c_prev),np.multiply(it,cct))
    ot = sigmoid(np.add(np.matmul(Wo,concat),bo))
    #remember here is element wise multiply *
    a_next = np.multiply(ot,np.tanh(c_next))
    
    # Compute prediction of the LSTM cell (≈1 line)
    yt_pred = softmax(np.add(np.matmul(Wy,a_next),by))
    ### END CODE HERE ###
